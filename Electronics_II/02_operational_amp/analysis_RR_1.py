...
filename_in  = "./data/data_R2_1.txt" 
filename_out = "./data/data_R2_1_out.txt"
filename_psp = "./data/pspice/rr2_32.85.txt"
#------------------------------------------------
# analysis
# import data for r2=32.85
...
deltaAdb   = (20.0/math.log(10)) * deltaAmp / amp
...
ax1.set_ylabel(r'$A_{db}=20\cdot log_{10}(V_o/V_i)$', labelpad=0.)
...
#find f_b
    #plot lines
ax1.plot([15.77, 15.77], [0, 28.13], color='black', lw=0.7)
ax1.plot([0, 15.77], [28.13, 28.13], color='black', lw=0.7)
    #add tickers
...
ax1.set_xticks( list(xticks) + [15.77]); ax1.set_xticklabels(list(xticks_lab) + ["15.77"])
    # plots the point
ax1.scatter(15.77, 28.13, color="red", marker='*', label="$Point, \\text{ } (f_b, A_{db})=(15.77kHz, 28.13db)$")


ax1.legend(loc=6)
ax1.grid()
ax1.set_xscale("log")
fig1.savefig("./plots/amplifier_RR1_Adb.pdf")
#-------------------------------------------------------
#-- phi - f 
fig2 = plt.figure()
...
ax2.grid()
ax2.set_xscale("log")
fig2.savefig("./plots/amplifier_RR1_phi.pdf")
#-----------------------------------------------------------
#plt.show()